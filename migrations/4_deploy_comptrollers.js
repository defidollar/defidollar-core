...
module.exports = async function(deployer, network, accounts) {
    ...
    const dfd = await Reserve.new('DefiDollar DAO', 'DFD', 18)
    const ibDfdProxy = await deployer.deploy(ibDFDProxy)
    await deployer.deploy(ibDFD)
    await ibDfdProxy.updateImplementation(ibDFD.address)
    const ibDfd = await ibDUSD.at(ibDfdProxy.address)
    await ibDfd.setParams(dfd.address, comptroller.address, 9950) // 0.5% redeem fee
    await comptroller.addBeneficiary(ibDfdProxy.address, [7500, 2500]) // 75:25 revenue sharing
