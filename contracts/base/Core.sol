...
contract Core is Ownable {
  ...
  */
  ...
  function update_system_stats() public returns(uint protocol_income) {
    ...
    stakeLPToken.update_income_diff(protocol_income);
  }
  ...
  function get_inventory() public view returns (uint inventory) {
    ...
    for(uint i = 0; i < peaks_addresses.length; i++) {
      ...
      for (uint j = 0; j < peak.system_coin_ids.length; j++) {
        portfolio[peak.system_coin_ids[j]] = portfolio[peak.system_coin_ids[j]].add(peak_portfolio[j]);
      }
    }
  ...
  function whitelist_peak(address peak, uint[] calldata _system_coins) external onlyOwner {
